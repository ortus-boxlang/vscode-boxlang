...
import { Tag, TagContext, getTagPattern, parseTags } from "../entities/tag";
import { CommentContext, CommentType, commentRules } from "../features/comment";
...
export function isColdFusionFile(uri: Uri): boolean {
    return /cfc$|cfml$|cfm$/i.test(path.extname(uri.fsPath));
}

export function isScriptFile(uri: Uri): boolean {
    return /cfc$|bx$|bxs$/i.test(path.extname(uri.fsPath));
}

export function isTemplateFile(uri: Uri): boolean {
    return /cfm$|cfml$|bxm$/i.test(path.extname(uri.fsPath));
}
...
export function getCfScriptRanges(document: TextDocument, range?: Range): Range[] {
    ...
    let cfscriptTagPattern: RegExp = getTagPattern("cfscript");
    ...
    cfscriptTagPattern = getTagPattern("bx:script");
    cfscriptTagMatch = null;
    while (cfscriptTagMatch = cfscriptTagPattern.exec(documentText)) {
        const prefixLen: number = cfscriptTagMatch[1].length + cfscriptTagMatch[2].length + 1;
        const cfscriptBodyText: string = cfscriptTagMatch[3];
        if (cfscriptBodyText) {
    ...
    for (let offset = textOffsetStart; offset < textOffsetEnd; offset++) {
        ...
        if (commentContext.inComment) {
            ...
            if (isScript) {
                ...
                } else if (lineText.endsWith(commentRules.scriptLineComment)) {
                    commentContext = {
                        inComment: true,
                        activeComment: commentRules.scriptLineComment,
                        commentType: CommentType.Line,
                        start: previousPosition
                    };
                } else if (lineText.endsWith(commentRules.scriptBlockComment[0])) {
                    commentContext = {
                        inComment: true,
                        activeComment: commentRules.scriptBlockComment,
                        commentType: CommentType.Block,
                        start: previousPosition
                    };
                }
            } else if (lineText.endsWith(commentRules.tagBlockComment[0])) {
                commentContext = {
                    inComment: true,
                    activeComment: commentRules.tagBlockComment,
                    commentType: CommentType.Block,
                    start: position.translate(0, 1 - commentRules.tagBlockComment[0].length)
                };
