...
export default class CFMLWorkspaceSymbolProvider implements WorkspaceSymbolProvider {
    ...
    public async provideWorkspaceSymbols(query: string, _token: CancellationToken): Promise<SymbolInformation[]> {
        ...
        if (editor) {
            ...
            if (document && (document.languageId === CFML_LANGUAGE_ID || document.languageId === BL_LANGUAGE_ID)) {
                uri = document.uri;
            }
        }
        if (!uri) {
            ...
            for (const document of documents) {
                if (document.languageId === CFML_LANGUAGE_ID || document.languageId === BL_LANGUAGE_ID) {
                    uri = document.uri;
                    break;
                }
            }
