import { CancellationToken, ParameterInformation, Position, Range, SignatureHelp, SignatureHelpContext, SignatureHelpProvider, SignatureInformation, TextDocument, Uri, workspace, WorkspaceConfiguration } from "vscode";
...
import { namedParameterPattern, Parameter } from "../entities/parameter";
...
import { BackwardIterator, getStartSigPosition as findStartSigPosition, getClosingPosition, getPrecedingIdentifierRange, isContinuingExpression } from "../utils/contextUtil";
...
export default class CFMLSignatureHelpProvider implements SignatureHelpProvider {
    ...
    public async provideSignatureHelp(document: TextDocument, position: Position, _token: CancellationToken, _context: SignatureHelpContext): Promise<SignatureHelp | null> {
        const cfmlSignatureSettings: WorkspaceConfiguration = workspace.getConfiguration("boxlang.cfml.signature", document.uri);
