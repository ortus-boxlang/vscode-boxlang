...
import { CancellationToken, Color, ColorInformation, ColorPresentation, DocumentColorProvider, Range, TextDocument, TextEdit, WorkspaceConfiguration, workspace } from "vscode";
import { IPropertyData } from "../entities/css/cssLanguageTypes";
import { cssColors, cssDataManager } from "../entities/css/languageFacts";
...
export default class CFMLDocumentColorProvider implements DocumentColorProvider {
    ...
    public async provideColorPresentations(color: Color, context: { document: TextDocument, range: Range }, _token: CancellationToken | boolean): Promise<ColorPresentation[]> {
        ...
        const cfmlCompletionSettings: WorkspaceConfiguration = workspace.getConfiguration("boxlang.cfml.suggest", context.document.uri);
        const replaceComments = cfmlCompletionSettings.get<boolean>("replaceComments", true);
